...
function Dashboard() {
  ...
  const StatCard = ({ title, value, icon, color, subtitle }: any) => (
    <div className="bg-white rounded-3xl p-6 border border-gray-200 hover:border-blue-600/50 transition-all group hover:scale-105 duration-300">
  ...
  const CategoryCard = ({ title, value, icon, color, bgColor }: any) => (
    <div className="bg-white rounded-2xl p-4 border border-gray-200 hover:border-blue-600/50 transition-all">
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-3xl p-12 border border-gray-200 flex items-center justify-center min-h-100">
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Card */}
      <div className="bg-linear-to-br from-white to-white rounded-3xl p-8 border border-gray-200 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200 lg:col-span-1">
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <h4 className="text-sm font-medium text-gray-600 mb-3">Project Status</h4>
        ...
        {/* Recent Projects */}
        <div className="bg-white rounded-3xl p-6 border border-gray-200 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-black flex items-center gap-2">
          ...
          <div className="space-y-3">
            {stats.recentProjects.length > 0 ? (
              stats.recentProjects.map((project: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-200 hover:border-blue-600/50 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {stats.recentNews.length > 0 ? (
            stats.recentNews.map((news: any, idx: number) => (
              <div key={idx} className="bg-gray-50 p-5 rounded-2xl border border-gray-200 hover:border-yellow-600/50 transition-all group">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-black line-clamp-1">{news.title}</h4>
                    <p className="text-[10px] text-gray-600 mt-1 flex items-center gap-1">
                      <Calendar size={10} /> {news.date || 'No date'}
                    </p>
                  </div>
                </div>
                <p className="text-sm text-gray-600 line-clamp-2 pl-2 border-l-2 border-yellow-600/30">
                  {news.excerpt || 'No description available'}
                </p>
                <div className="mt-3 flex items-center justify-end">
                  <span className="text-[10px] px-2 py-1 bg-gray-200 rounded-lg text-gray-600">
            ...
            <div className="col-span-3 text-center py-12 bg-gray-50 rounded-2xl border border-gray-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-gray-200">
          ...
          <p className="text-2xl font-bold text-black">{stats.apartments}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-gray-200">
          <p className="text-gray-600 text-xs">Residencies</p>
          <p className="text-2xl font-bold text-black">{stats.residencies}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-gray-200">
          <p className="text-gray-600 text-xs">ROI Projects</p>
          <p className="text-2xl font-bold text-black">{stats.roiProjects}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-gray-200">
          <p className="text-gray-600 text-xs">Sold Out</p>
          <p className="text-2xl font-bold text-black">{stats.soldOutProjects}</p>
        </div>
      </div>
    </div>
  );
